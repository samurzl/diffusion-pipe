...
For a small dataset whose files have many different shapes or durations, set `unbucketed = true` at the root of the dataset TOML, provide exactly one target `resolutions` entry, and omit aspect-ratio settings, `frame_buckets`, and `size_buckets`. Each sample is resized to approximately `resolution²` pixels while keeping its native aspect ratio and usable duration; the preprocessor then applies H3's required alignment by rounding width and height to the nearest multiple of 32 and video length down to a multiple of 17. Videos shorter than 17 frames after conversion to 24 fps are skipped. Both `micro_batch_size_per_gpu` and `image_micro_batch_size_per_gpu` (and their eval equivalents for an unbucketed eval set) must resolve to `1`. Gradient accumulation and data parallelism remain supported: differently shaped samples are prepared as separate physical batches, and only the final incomplete logical batch across the whole dataset is dropped. NSYNC is supported by preparing its positive, negative, and anchor roles independently, subject to NSYNC's existing data-parallel-world-size-one restriction; see the [unbucketed H3 dataset example](../examples/minimax_h3_unbucketed_dataset.toml) and [unbucketed NSYNC example](../examples/minimax_h3_unbucketed_nsync_dataset.toml).
